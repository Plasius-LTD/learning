...
### Local Notes
This repo is `@plasius/learning`, the infrastructure-neutral TypeScript contract and validation package for Plasius learning products.
...
- Keep the package free from HTTP, persistence, authentication, economy, cloud, model-provider, code-execution, and UI dependencies.
- Treat published path and module versions as immutable contracts.
- Deterministic assessment remains the sole score authority; agent contracts cannot award scores, completion, badges, publishing, or hardware approval.
